...
export function getPoints({ x, y, width, height, rotation }, zoom) {
  ...
  const rect = new Matrix(
    [[-wc, hc], [0, hc], [wc, hc], [wc, 0], [wc, -hc], [0, -hc], [-wc, -hc], [-wc, 0]]);
  // eslint-disable-next-line no-mixed-operators,max-len
  return deg.dot(rect.T()).T().valueOf().map(item => ({ x: item[0] + wc + x * zoom, y: -(item[1] - hc) + y * zoom }));
}
...
export function getBoundingRect(transform) {
  ...
  const points = getPoints(transform);

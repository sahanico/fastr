  ...
  <div id="workflows">
    ...
    <div>
      <div class="headline red--text pb-5 pt-5" style="text-align: center">
        {{context === 'create'? 'Create ' : 'Update '}}Workflow
      </div>
      <form @submit.prevent="onSubmit">
        ...
        <v-card class="pa-5 ma-5 pb-9">
          <v-row v-for="(item, idx) in design.meta.pages" :key="idx">
            <v-col cols="12" md="3">
              <v-text-field v-model="design.meta.pages[idx].label" label="Label"/>
            </v-col>
            <v-col cols="12" md="3">
              <v-text-field v-model="design.meta.pages[idx].name" label="Name" />
            </v-col>
            <v-col cols="12" md="3">
              <v-select v-model="design.meta.pages[idx].form"
                        :items="forms" label="Form"/>
            </v-col>
            <v-col cols="12" md="3">
              <v-select v-model="design.meta.pages[idx].process"
                        :items="processes" label="Post Process"/>
            </v-col>
          </v-row>
          <v-btn style="position: absolute; right: 16px" icon
                 @click="design.meta.pages.push({
                      name: '',
                      label: '',
                      form: '',
                      process: '',

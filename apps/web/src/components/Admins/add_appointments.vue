      ...
      <v-layout>
        <v-flex class="mb-5 pb-5" offset-sm4 sm4 xs12>
          <v-stepper v-model="e6" vertical>
            ...
            <v-stepper-step :complete="e6 > 1" :editable="stepTwoEditable"
                            color="red" step="2">Pick an account manager
              <small v-show="e6 > 2">{{ personSelected() }}</small>
            </v-stepper-step>
            <v-stepper-content step="2">
              <div>
                <v-btn v-show="showHarsh" :color="harshColor" dark text @click="selectHarsh()">
                  <v-flex align-left class="mb-5" justify-left layout>
                    <div class="subheading" style="marginTop: 10px"> Harsh Sahani</div>
                  </v-flex>
                </v-btn>
                <v-btn v-show="showRakesh" :color="rakeshColor" dark text @click="selectRakesh()">
                  <v-flex align-left class="mb-5" justify-left layout>
                    <div class="subheading" style="marginTop: 10px"> Rakesh Chaudhry</div>
                  </v-flex>
                </v-btn>
            ...
            <v-stepper-step :complete="e6 > 2" :editable="stepThreeEditable"
            ...
            <v-stepper-step :complete="e6 > 4" :editable="stepFourEditable"
              ...
              <small v-show="e6 > 4">{{ bookedBySmall }}</small></v-stepper-step>
            <v-stepper-content step="4">
              <v-flex xs12>
                <div><input v-model="systemUser" type="checkbox"/>
                  <span><label></label>System User</span>
                </div>
              </v-flex>
              <v-flex v-if="!systemUser" xs12>
                <v-text-field v-model="bookedBy" label="Name" @click="nameItemChange()">
                </v-text-field>
              </v-flex>
              <v-flex v-if="systemUser" xs12>
                <v-autocomplete v-model="selectedUser" :items="autocompleteUsers"
                                chips dense label="Booked By" outlined
                                return-object small-chips>
                </v-autocomplete>
              </v-flex>
              <v-btn color="red" dark @click="nameContinueBtnClick()">Continue</v-btn>
            </v-stepper-content>

            <v-stepper-step :complete="e6 > 5" color="red" step="5">
  ...
  methods: {
    ...
    submitBtnClick() {
      ...
      const appointment = {
        ...
        userId: this.currentAppointment.bookedByUserId,
      };
      this.successDialog = true;
      return this.$store.dispatch('createAppointmentAsAdmin', appointment);
    },
  },
};
</script>
